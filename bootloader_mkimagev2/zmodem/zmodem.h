...
size_t zmodem_receive(const char *directory,
		      bool (*approver)(const char *filename, size_t size, unsigned int date),
		      bool tick_cb(const char *fname, long bytes_sent, long bytes_total, long last_bps, int min_left, int sec_left),
		      void (*complete)(const char *filename, int result, size_t size, unsigned int date),
		      uint64_t min_bps,
		      uint32_t flags);
...
size_t zmodem_send(int file_count,
		   ...
		   void (*complete)(const char *filename, int result, size_t size, int date),
		   uint64_t min_bps,
		   uint32_t flags);
